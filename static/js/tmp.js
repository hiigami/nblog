...
var eApp = eApp || { host: location.protocol + '//' + location.host, val: 0, myfile:{}};
...
eApp.generalClickHandler = function(){
	...
	$("form").submit(function (e) {
        ...
        if ($('input[name=slug]',$_form).val() == "") {
        	...
        	newdata += "&slug="+Math.random().toString(36).substring(8)+"&createdAt="+new Date()+"&votes=0&active=true";
        }
        ...
        if ($_form.find('input[type=file]').length > 0) {
            /*var newdata = new FormData($_form[0]);
            _contentType = false;
            _processData = false;*/
            var _tmp = $("#base").text().replace(/data:image\/jpeg;base64,/g,"");
            newdata += "&file=" + _tmp;
        }
